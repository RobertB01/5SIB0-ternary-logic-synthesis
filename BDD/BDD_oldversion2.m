...
function tc = BDD3(O, X)
    ...
    tc = CountTransistors(nodes, edges, X);
                ...
                if(numel(unique(O_new(:,2))) == 1) %constant function
                    ...
                    elseif(O_new(1,2)  == 0 & O_new(2,2)  == 0 & O_new(3,2)  == 2)
                        ...
                        if(o0 == o1) %proposition 3 check
                          ...
                          edge_left_dest = o2;
                          edge_right_dest = o1;
                          updateGraph_small(nodes, edges, X_new{1}, nodes{i,2}+1, edge_left, edge_right, edge_left_dest, edge_right_dest);
                        
                        elseif(o1 == o2) %propisiton 2 holds or it does not matter wether we choose left or right according to the algorithm
                          edge_left = "_is_0?";
...
function tc = CountTransistors(nodes, edges, X)
    nti_pti_list = zeros(length(X)-1,2); % 1 for nti, 2 for pti
