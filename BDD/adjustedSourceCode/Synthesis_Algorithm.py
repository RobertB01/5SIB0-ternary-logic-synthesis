...
def in_args(table_array, n_inputs, n_outputs):
    inputs=int(n_inputs)
    ...
    for i in range(outputs):
        for j in range(comb):
            out[i].append(table_array[j][inputs+i])
    print(out)
    ...
    print('Transistor Count (with PTI NTI binot inverters):', Final_Mux2*4+4*inputs+4)
    print('graph: \n', BTDD_Total_Graph)
    #print(BTDD_Total_Graph)
    return BTDD_Total_Graph,In_Cost2x,Cost,Final_Mux2,Final_Mux2*4